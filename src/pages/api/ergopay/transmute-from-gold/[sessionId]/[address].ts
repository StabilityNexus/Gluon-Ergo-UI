...
import { handleTransmuteFromGoldSwapErgoPay } from "@/lib/functions/reactor/handleTransmutation";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await handleTransmuteFromGoldSwapErgoPay({
      gluonInstance,
      gluonBoxJs: gluonBox,
    ...
    if (!result.reducedTx) {
      console.error("[ErgoPay Transmute-from-Gold] Handler returned no reducedTx");
    ...
    return res.status(200).json({
      reducedTx,
      address,
      message: `Transmutation: ${session.fromAmount} GAU → ${session.toAmount} GAUC`,
      messageSeverity: "INFORMATION",
      replyTo: replyToUrl,
    });

  } catch (error: any) {
    console.error(`Error processing Transmute from Gold transaction:`, error);
